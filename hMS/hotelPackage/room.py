...
class Roombooking:
    def __init__(self, root):
        ...
        lbl_title = Label(self.root, text="ROOMBOOKING DETALILS", font=("times new roman", 18, "bold"), bg="black",fg="gold", bd=4, relief=RIDGE)
        ...
        # =================== Fetch data btn ==========================
        btnFatchData = Button(lableframeleft,command=self.Fetch_contact,text="Fetch Data" , font=("arial", 8, "bold"), bg="black",fg="Gold", width=8)
        btnFatchData.place(x=347,y=4)

        # Check_in Date
        check_in_date= Label(lableframeleft, text="Check_In Date:", font=("arial", 12, "bold"), padx=2, pady=6)
        check_in_date.grid(row=1, column=0, sticky=W)

        txt_check_in_date= ttk.Entry(lableframeleft,textvariable=self.var_checkin,width=29, font=("arial", 13, "bold"), )
        txt_check_in_date.grid(row=1, column=1)

        # Check_out Date
        lbl_Check_out = Label(lableframeleft, text="Check_Out Date:", font=("arial", 12, "bold"), padx=2, pady=6)
        lbl_Check_out.grid(row=2, column=0, sticky=W)
        txt_Check_out = ttk.Entry(lableframeleft,textvariable=self.var_checkout,width=29, font=("arial", 13, "bold"), )
        ...
        combo_RoomType = ttk.Combobox(lableframeleft,textvariable=self.var_roomtype,font=("arial", 13, "bold"), width=27, state="readonly")
       ...
       # txtRoomAvailable = ttk.Entry(lableframeleft,textvariable=self.var_roomavailable,font=("arial", 13, "bold"), width=29)
        ...
        combo_RoomNo= ttk.Combobox(lableframeleft, textvariable=self.var_roomavailable, font=("arial", 13, "bold"),width=27, state="readonly")
        ...
        # Total Cost
        lblTotalCost = Label(lableframeleft, font=("arial", 12, "bold"), text="Total Cost:", padx=2, pady=6)
        ...
        #  ==================== Bill Button ===============
        btnBill = Button(lableframeleft, text="Bill",command=self.total,font=("arial", 11, "bold"), bg="black", fg="light green", width=10)
        ...
        btnAdd = Button(btn_frame, text="INSERT",command=self.add_data,font=("arial", 11, "bold"), bg="black",fg="Gold", width=10)
        btnAdd.grid(row=0, column=0, padx=1)

        btnUpdate = Button(btn_frame, text="UPDATE",command=self.update,font=("arial", 11, "bold"), bg="black",fg="Blue", width=10)
        btnUpdate.grid(row=0, column=1, padx=1)

        btnDelete = Button(btn_frame, text="DELETE",command=self.mDelete,font=("arial", 11, "bold"), bg="black",fg="Red", width=10)
        btnDelete.grid(row=0, column=2, padx=1)

        btnReset = Button(btn_frame, text="RESET",command=self.reset,font=("arial", 11, "bold"), bg="black", fg="Green", width=10)
        btnReset.grid(row=0, column=3, padx=1)

        # ================== Right Side Image ================================
        img3 = Image.open(r"C:\Users\Isuru Dilshan\Desktop\python\hotel MS\hMS\hotelPackage\image\bed2.jpg")
    ...
    def add_data(self):
            if self.var_contact.get() == "" or self.var_checkin.get() == "":
                ...
                try:
                    conn = mysql.connector.connect(host="localhost", username="root", password="7986@1223Id",
                                                   database="management")
                    my_cursor = conn.cursor()
    ...
    def update(self):
        if self.var_contact.get() == "":
            ...
            self.fetch_data()
            conn.close()
            messagebox.showinfo("Update", "Room details has been update succesfully", parent=self.root)
    ...
    def search(self):
        ...
        my_cursor.execute(
            "select * from room where " + str(self.search_var.get()) + " LIKE '%" + str(self.txt_search.get()) + "%'")
        rows = my_cursor.fetchall()
        if len(rows) != 0:
            self.room_table.delete(*self.room_table.get_children())
            for i in rows:
